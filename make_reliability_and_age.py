...
# what the single-rated lessons cost: reliability if every lesson had two raters,
# and whether single-rated lessons score differently
rel2 = {y: dec[y]["school"] / (dec[y]["school"] + dec[y]["lesson"] / dec[y]["m_l"] + dec[y]["rater"] / (dec[y]["m_l"] * 2.0))
        for y in ("W1", "S1")}
nr = L.groupby("lesson")["rater"].nunique(); L["nr"] = L["lesson"].map(nr)
gap1 = {y: L[L.nr == 1][y].mean() - L[L.nr >= 2][y].mean() for y in ("W1", "S1")}
schs = L.groupby("school").agg(share1=("nr", lambda v: (v == 1).mean()), W1=("W1", "mean"), S1=("S1", "mean"))
n_sch_single = int((schs.share1 > 0).sum())
# per-school share of single-rated lessons, keyed on URN, for the Chapter 3 robustness row
share_out = (schs.assign(urn=schs.index.map(vmap))[["urn", "share1"]]
             .rename(columns={"share1": "share_single_rated"}).dropna(subset=["urn"]))
share_out["n_lessons"] = L.groupby("school")["lesson"].nunique().reindex(schs.index).values
SHARE_OUT = ROOT / "scores" / "visit_single_rated_share.csv"
_cur = SHARE_OUT.read_text(encoding="utf-8") if SHARE_OUT.exists() else ""
_new = share_out.to_csv(index=False, lineterminator="\n")
if _cur != _new:
    SHARE_OUT.write_text(_new, encoding="utf-8", newline="\n"); print("wrote", SHARE_OUT.name)
corr1 = {y: schs.share1.corr(schs[y]) for y in ("W1", "S1")}
print(f"two-rater reliability W1 {rel2['W1']:.2f} S1 {rel2['S1']:.2f}; single-rated gap W {gap1['W1']:.2f} S {gap1['S1']:.2f}; "
      f"schools with single-rated lessons {n_sch_single}; corr share-single x score W {corr1['W1']:.2f} S {corr1['S1']:.2f}")
# rank-based gap statistics (20 Aug 2026): same-school comparisons of where the head
# and the observers place the school in their own distributions; scale-free, unlike a
# difference in levels between two different instruments
bq = d.dropna(subset=["gs_warmth_enacted", "gs_warmth_espoused", "gs_strictness_enacted", "gs_strictness_espoused"]).copy()
rankq = {}
for dim, en, es in (("W", "gs_warmth_enacted", "gs_warmth_espoused"), ("S", "gs_strictness_enacted", "gs_strictness_espoused")):
    qe = pd.qcut(bq[en].rank(method="first"), 4, labels=False); qs = pd.qcut(bq[es].rank(method="first"), 4, labels=False)
    rankq[dim] = dict(top_n=int((qs == 3).sum()), top_match=int(((qe == 3) & (qs == 3)).sum()),
                      top_below_median=int(((qs == 3) & (bq[en] < bq[en].median())).sum()),
                      same_q=float((qe == qs).mean()), rho=float(stats.spearmanr(bq[en], bq[es])[0]))
print("rank gap", rankq)
...
# Stratified alpha for the enacted composite (referee point, 21 Aug 2026):
# Spearman-Brown treats the in-lesson and outside halves as parallel forms; if
# the settings are distinct facets, the composite reliability is the stratified
# coefficient instead, computed from the halves' own reliabilities and their
# correlation (standardised halves, equal weights).
def strat_alpha(rel1, rel2, r12):
    return 1.0 - (0.25 * (1 - rel1) + 0.25 * (1 - rel2)) / (0.5 + 0.5 * r12)
rel_enW_up = strat_alpha(dec["W1"]["rel"], odec["W2"]["rel"], r_w_halves)
rel_enS_up = strat_alpha(dec["S1"]["rel"], odec["S2"]["rel"], r_s_halves)

# Expected top-quartile match under the SAME-CONSTRUCT null (referee point):
# if heads and observers measured one construct with the stated reliabilities,
# the observable correlation is capped at sqrt(rel_enacted x alpha_espoused),
# and the expected number of the 26 espoused-top-quartile schools also in the
# enacted top quartile follows from the bivariate normal. Simulated, seed fixed.
_rng = np.random.default_rng(20260821)
def expected_topq_match(r_cap, n_top=26, n=2_000_000):
    x = _rng.standard_normal(n)
    y = r_cap * x + np.sqrt(1 - r_cap**2) * _rng.standard_normal(n)
    q = np.quantile(x, 1 - 26/103); qy = np.quantile(y, 1 - 26/103)
    p_match = ((x > q) & (y > qy)).sum() / (x > q).sum()
    return n_top * p_match
cap_w = np.sqrt(rel_enW * alpha_w3); cap_s = np.sqrt(rel_enS * alpha_s4)
exp_match_w = expected_topq_match(cap_w)
exp_match_s = expected_topq_match(cap_s)


def expected_half_match(r_cap, n_school=103, n=2_000_000):
    """Expected same-half agreement if the two scores were noisy readings
    of one trait correlated at r_cap (median split, strict >)."""
    x = _rng.standard_normal(n)
    y = r_cap * x + np.sqrt(1 - r_cap**2) * _rng.standard_normal(n)
    p = ((x > np.median(x)) == (y > np.median(y))).mean()
    return n_school * p


# Halves count (27 Aug 2026): which half of the sample is the school in?
# Strict > median for the headline; >= median as the tie-break alternate
# (the espoused scales are coarse, so many schools sit exactly at the
# median). Luck benchmark by permutation, honouring the actual marginals.
half = {}
_hrng = np.random.default_rng(20260828)
for _dim, _en, _es, _cap in (("W", "gs_warmth_enacted", "gs_warmth_espoused", cap_w),
                             ("S", "gs_strictness_enacted", "gs_strictness_espoused", cap_s)):
    _e = bq[_en].values; _s = bq[_es].values
    _he = _e > np.median(_e); _hs = _s > np.median(_s)
    _match = int((_he == _hs).sum())
    _alt = int(((_e >= np.median(_e)) == (_s >= np.median(_s))).sum())
    _luck = float(np.mean([(_he == _hs[_hrng.permutation(len(_hs))]).sum()
                           for _ in range(4000)]))
    half[_dim] = dict(match=_match, alt=_alt, luck=_luck,
                      ceil=expected_half_match(_cap))
half_luck = round((half["W"]["luck"] + half["S"]["luck"]) / 2)
print(f"halves count: W {half['W']['match']} (alt {half['W']['alt']}, "
      f"luck {half['W']['luck']:.1f}, ceil {half['W']['ceil']:.0f}); "
      f"S {half['S']['match']} (alt {half['S']['alt']}, "
      f"luck {half['S']['luck']:.1f}, ceil {half['S']['ceil']:.0f})")
print(f"stratified-alpha composite rel W {rel_enW_up:.2f} S {rel_enS_up:.2f}; "
      f"same-construct expected top-quartile match W {exp_match_w:.0f} S {exp_match_s:.0f} of 26")
...
# ---- the two interview exclusions (Ch2 measurement section) ----
# Both are justified in the chapter by a correlation with the enacted
# scores: the articulacy ratings measure the head rather than the
# school, and the systems count measures what a school names rather
# than how it runs.
for _c in ('gs_ht_articulacy', 'gs_S3', 'gs_teaching_enacted'):
    if _c in d.columns:
...
artic_w = _r('gs_ht_articulacy', 'gs_warmth_enacted')
artic_s = _r('gs_ht_articulacy', 'gs_strictness_enacted')
artic_t = _r('gs_ht_articulacy', 'gs_teaching_enacted')
systems_s = _r('gs_S3', 'gs_strictness_enacted')
artic_esp_w = _r('gs_ht_articulacy', 'gs_warmth_espoused')
artic_esp_s = _r('gs_ht_articulacy', 'gs_strictness_espoused')

nums = ("% Auto-generated by thesis/make_reliability_and_age.py -- do not edit by hand.\n"
        + mac("RelWLessons", f"{dec['W1']['rel']:.2f}") + mac("RelSLessons", f"{dec['S1']['rel']:.2f}")
        + mac("RelTLessons", f"{dec['T1']['rel']:.2f}") + mac("RelWOutside", f"{odec['W2']['rel']:.2f}") + mac("RelSOutside", f"{odec['S2']['rel']:.2f}")
        ...
        + mac("ShareRaterW", pct(dec['W1']['rater'])) + mac("ShareRaterS", pct(dec['S1']['rater']))
        + mac("RelWOneTwoRaters", f"{rel2['W1']:.2f}") + mac("RelSOneTwoRaters", f"{rel2['S1']:.2f}")
        + mac("SingleRatedGapW", f"{gap1['W1']:.2f}") + mac("SingleRatedGapS", f"{gap1['S1']:.2f}")
        + mac("SchoolsWithSingleRated", str(n_sch_single)) + mac("CorrSingleShareW", f"{corr1['W1']:.2f}")
        ...
        + mac("RelEnactedWUpper", f"{rel_enW_up:.2f}") + mac("RelEnactedSUpper", f"{rel_enS_up:.2f}")
        + mac("TopQExpectedSameW", f"{exp_match_w:.0f}") + mac("TopQExpectedSameS", f"{exp_match_s:.0f}")
        + mac("DisSplitW", f"{dis_w:.2f}") + mac("DisSplitS", f"{dis_s:.2f}") + mac("DisOfstedS", f"{dis_ofs:.2f}")
        + mac("TopQN", str(rankq["W"]["top_n"])) + mac("TopQWMatch", str(rankq["W"]["top_match"]))
        + mac("TopQWBelowMedian", str(rankq["W"]["top_below_median"])) + mac("TopQSMatch", str(rankq["S"]["top_match"]))
        + mac("TopQSBelowMedian", str(rankq["S"]["top_below_median"]))
        + mac("HalfMatchW", str(half["W"]["match"])) + mac("HalfMatchWAlt", str(half["W"]["alt"]))
        + mac("HalfMatchS", str(half["S"]["match"])) + mac("HalfLuck", str(half_luck))
        + mac("HalfCeilingW", f"{half['W']['ceil']:.0f}") + mac("HalfCeilingS", f"{half['S']['ceil']:.0f}")
        + mac("SameQuartileW", pct(rankq["W"]["same_q"])) + mac("SameQuartileS", pct(rankq["S"]["same_q"]))
        + mac("RhoSplitW", f"{rankq['W']['rho']:.2f}") + mac("RhoSplitS", f"{rankq['S']['rho']:.2f}")
        ...
        + mac("ArticEnW", f"{artic_w:+.2f}") + mac("ArticEnS", f"{artic_s:+.2f}")
        + mac("ArticEnT", f"{artic_t:+.2f}") + mac("SystemsEnS", f"{systems_s:+.2f}")
        + mac("ArticEspW", f"{artic_esp_w:+.2f}") + mac("ArticEspS", f"{artic_esp_s:+.2f}")
        + (mac("CILooStrictLo", f"{loo_ci[0]:.2f}") + mac("CILooStrictHi", f"{loo_ci[1]:.2f}") if loo_ci is not None else ""))

ap = argparse.ArgumentParser(); ap.add_argument("--check", action="store_true"); a = ap.parse_args()
rc = 0
tab, _ = move_caption_above(tab)
# caption convention (PIPELINE.md, 29 Aug 2026)
tab = caption_to_title(tab, 'Variance components and score reliability')
