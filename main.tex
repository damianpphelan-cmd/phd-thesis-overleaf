...
The first paper measures culture. I visited \NTierOne{} schools. In each I carried out a day of structured
observation and an interview with the headteacher. The observation measured
what the school does; the interview measured what the head says it does.
Both covered how warm the school is towards its pupils and how strictly it
runs. Every
school's inspection report, behaviour policy and website is then analysed in
two ways: a prediction model learns which words and phrases are linked with
warmer or stricter schools, and a large language model scores each document
against a one-page mark scheme. Scores read from inspection reports agree modestly with the visit scores
for both warmth and strictness. Warmth shows in a report's everyday
wording, and the prediction model picks it up from word frequencies alone.
Strictness does not. It appears only in specific statements, for example
that conduct standards are consistently enforced, and only the language
model reading with the mark scheme finds those. What a headteacher says
about the school agrees only weakly with what observers find there. The
disagreement is largest for warmth. A school's own documents agree with
what the head says rather than with what observers record.

The second paper relates the visit scores to Progress~8. A
standard deviation of warmth, as scored on the visit, is associated with
\PrimarySpecWSD{} of a standard deviation of Progress~8, and a standard
deviation of strictness with \PrimarySpecSSD{}, and controlling for the quality
of the teaching observed does not reduce either association. Warmth predicts progress only when it is measured by
observation: using the headteacher's own rating of the school in place of the
observer's score removes the warmth association, whereas strictness is
associated with progress whether it is measured by the observer or by the
headteacher. Scoring warmth and strictness from inspection reports instead of visits
shows the same associations with Progress~8 across effectively all English
...
suitably qualified teacher. During the period we study, fewer than half of
state schools offered the subject. When a school starts offering the
subject, the share of its pupils who take it rises by 5.7 percentage
points, and taking the subject raises earnings at age 28 by around 14 per
cent, an implied net lifetime gain of \pounds 65,000--\pounds 90,000 for
each student who takes it only because it is offered. The extra tax raised
from those pupils' higher earnings exceeds the cost of employing the
teacher, and if every school offered the subject, a larger share of the
pupils taking it would come from outside London and the South East.
