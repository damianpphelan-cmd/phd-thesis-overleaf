...
\smallskip
\footnotesize
\textit{Notes:} Each row reports the first-stage coefficient, the IV estimate of the effect of A-level economics on log earnings at age 28, and the first-stage $F$-statistic. Row~1 is the baseline with linear individual controls. Row~2 uses the saturated covariate specification with region entered as a binary London--SE indicator (sex $\times$ ethnicity group $\times$ FSM $\times$ GCSE tercile $\times$ London-SE); the main-text saturated specification instead uses the three-category region (London/South East, rest of England, missing). Row~3 adds school-specific linear time trends. Row~4 replaces cohort FE with region $\times$ year FE. Row~5 replaces GCSE attainment with KS2 (age 11) attainment in the saturated cells. Row~6 divides GCSE attainment into twenty equal groups instead of three. Row~7 uses the double/debiased machine learning method of \citet{chernozhukov2018dml}. Instead of entering the controls as linear terms, a penalised regression (lasso) predicts both take-up and availability from the controls. To avoid overfitting, the predictions for each student are made from a model fitted on the other students. The effect of take-up on earnings is then estimated from the parts of take-up and availability that the controls do not predict. This method has no single first-stage coefficient on availability, so no first-stage $F$ is reported. Standard errors clustered at the school level in parentheses. Sample: students in switcher schools, GCSE cohorts 2002--2008. The observation count refers to the baseline; the saturated specifications use slightly fewer observations, because combinations of characteristics with too few students cannot be estimated and those students are excluded (cf.\ Table~\ref{tab:main} column~4).
