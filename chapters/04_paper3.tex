...
Taking A-level economics operates on two margins: it raises university attendance by 14.0 percentage points (Appendix Table~\ref{tab:other_outcomes}) and economics degree take-up by 15.2 percentage points (Table~\ref{tab:degree_channel}). Valuing the attendance margin at the average net lifetime return of \pounds 99,000 and the switching margin at the gap between the economics return (\redbf{\pounds 570,000} when the published returns for men and women are weighted by the composition of economics students) and the returns to the subjects compliers would otherwise study \redbf{(around \pounds 75,000)} implies a net individual return of approximately \redbf{\pounds 90,000} per induced student. The same components applied to the gross and exchequer schedules imply a gross return of roughly \redbf{\pounds 190,000} and an exchequer return of roughly \redbf{\pounds 100,000} (Table~\ref{tab:main} Panel~C). As a check on the order of magnitude, assuming instead that the 14\% age-28 earnings premium persists over the lifecycle gives a net gain of around \redbf{\pounds 65,000}.
