...
% Result 2: Impact
Second, we estimate the causal effect of taking A-level economics on earnings. The first stage is strong: within-school availability of A-level economics increases the probability of taking the subject by 5.7 percentage points ($F = 843$). Students induced to take A-level economics are 15.2 percentage points more likely to study economics at university, confirming the gateway role documented in our descriptive analysis. The IV estimate implies that taking A-level economics raises earnings at age 28 by around 14\%. Using lifetime returns predicted from an external model following the methodology of \citet{britton2020degrees}, we estimate that the gain in expected lifetime returns is approximately \redbf{\pounds 45,000} in present value (gross) and \redbf{\pounds 30,000} net of tax and student loan repayments.
...
The earnings effect is partly explained by a shift into economics degrees, which have among the highest lifetime returns of any subject: the 15.2 percentage point increase in economics degree take-up among compliers, combined with the estimated return to an economics degree of approximately \redbf{\pounds 250,000} (Britton et al.\ 2020), accounts for roughly \redbf{85\%} of the IV estimate.

% Result 3: Policy
Third, we compute the return to expanding access. If all schools offered A-level economics, our estimates imply an additional \redbf{4,200} students per cohort would take A-level economics, of whom approximately \redbf{640} would go on to study economics at university, shifting the demographic composition of the intake towards students from disadvantaged backgrounds and regions outside London and the South East. We estimate an MVPF of hiring an additional economics teacher of \redbf{2.0}: each pound of net government spending generates around \redbf{two} pounds of private lifetime earnings gains. Because the supply constraint binds most tightly for students from disadvantaged backgrounds, removing it simultaneously improves allocative efficiency and reduces inequality: the policy has no equity-efficiency trade-off.
...
Most students --- \redbf{around 80\%} --- attend the same secondary school from age 11 to 18, though at the end of GCSE (age 16) some transfer to a different institution for A-levels, either by choice or because their school does not offer post-16 education. We discuss the implications of school-changing for identification in Section~\ref{sec:Impact}.
...
To put a scale on the misallocation, we apply the access gaps in Table~\ref{tab:access_takeup} to current cohort sizes. Equalising access across groups would generate approximately \redbf{4,200} additional A-level economics students per cohort: an increase of \redbf{28}\% relative to current numbers. The demographic composition of A-level economists would shift correspondingly, with the share from FSM backgrounds rising from \redbf{8}\% to \redbf{9}\%, and the share from outside London and the South East
from \redbf{56}\% to \redbf{60}\%.
...
The age-28 estimates capture only the early-career return to A-level economics. To assess lifetime effects we draw on an external lifetime-returns model estimated at the Institute for Fiscal Studies \redbf{[cite the 2026 IFS report]}, which follows the methodology of \citet{britton2020degrees}: for each degree subject it estimates the lifetime return relative to a no-university counterfactual, under selection on observables, simulated to retirement. Rather than rebuilding that model, we use a coefficients approach. Three auxiliary regressions in the external data relate finished per-person lifetime returns to background characteristics and degree-subject dummies, and the resulting coefficient vectors are applied to our sample. This generates three predicted outcomes per person: the gross lifetime return; the net individual return after income tax, National Insurance and Plan~5 student loan repayments; and the exchequer return, which nets the extra tax and loan repayments against fee loans, maintenance loans and teaching grants. Each is a gain relative to not attending university, in 2025/26 prices and discounted to age 18 at Green Book rates, and non-graduates are assigned zero by construction.

Each predicted outcome then replaces log earnings in the IV specification of equations~\eqref{eq:firststage} and~\eqref{eq:secondstage}, so the coefficient reads directly in pounds: the effect of taking A-level economics on the expected lifetime return, combining an extensive margin (attending university at all) and a subject-switching margin.\footnote{Details of the prediction step and its diagnostics are in Appendix~\ref{sec:Appendix_Simulation}.} Three caveats apply. The predictions are generated regressors, so the second-stage standard errors ignore first-step estimation error. They are shrunk towards the mean, which attenuates the estimates towards zero. And the method prices only the degree-choice channel: Panel~B of Table~\ref{tab:degree_channel} shows that much of the age-28 return arises among compliers who do not change degree subject, and that component is absent from these figures by construction, so Panel~C is a floor on the lifetime effect.

The IV estimate of the gross lifetime return from taking A-level economics is approximately \redbf{\pounds 45,000} in present value (Table~\ref{tab:main} Panel C). The net individual return is \redbf{\pounds 30,000}, and the exchequer return is \redbf{\pounds 15,000} per student. This fiscal figure, along with analogues constructed for different subgroups (Section~\ref{sec:heterogeneity} below), is the key input to the MVPF calculation in Section~\ref{sec:Policy}.

The saturated specification gives a gross lifetime return of \redbf{\pounds 48,000}, close to the linear estimate, so the weighting concern of \citet{blandhol2022tsls} matters no more for lifetime earnings than it does for earnings at age 28.
...
Figure~\ref{fig:heterogeneity} reports the IV estimate of the effect of A-level economics on log earnings at age 28 (Panel A) and lifetime earnings (Panel B), estimated separately for subgroups defined by sex, FSM eligibility, region, and ethnicity. Each estimate uses the same specification as the main result, with the sample restricted to the relevant subgroup.

\input{figures/ch4/fig_heterogeneity}

Three patterns stand out in Panel A. First, the point estimate for women (0.335) is much larger than for men (0.051): the women induced into economics by availability are fewer, but those who are induced appear to gain substantially more, which strengthens the case for supply-side expansion as a gender-equity intervention. Second, the estimate is larger in the rest of England (0.148) than in London and the South East (0.110): it is not that London students benefit more from economics, they are simply more likely to have the opportunity. Third, the largest estimate is for Black African and Caribbean students (0.660), one of the groups whose access a supply expansion would disproportionately improve, while the estimates for Indian and Chinese students --- who already have high access and take-up --- are small, consistent with diminishing returns at higher levels of access.

The FSM estimate is small (0.034) but very imprecise: its confidence interval runs from $-0.38$ to $0.45$, so the data cannot distinguish a zero return from a return three times the overall estimate. The equity case for expansion therefore rests on the access evidence of Section~\ref{sec:Access} rather than on any demonstrated difference in returns for disadvantaged students. Where the subgroup estimates are informative --- sex, region, and ethnicity --- the returns are, if anything, concentrated among the groups with the least access, the combination of unequal access and larger returns that generates the efficiency-equity complementarity motivating Section~\ref{sec:Policy}. Propensity-score heterogeneity, which splits the sample by the predicted propensity to take A-level economics rather than by demographic group, is reported in Appendix Table~\ref{tab:propensity_heterogeneity}.

These subgroup estimates are not only descriptive: they are direct inputs to the policy analysis in Section~\ref{sec:Policy}. The MVPF of hiring an economics teacher depends on the returns earned by the students whose choices are shifted, which in turn depend on the composition of the schools where teachers are hired. Because never-offer schools serve a more disadvantaged population than the switcher schools generating our main estimates (Figure~\ref{fig:external_validity}), we use the subgroup-specific estimates to construct a composition-adjusted return that reflects the observable characteristics of students in never-offer schools, following the approach of \citet{angrist2013extrapolate}. This adjustment is quantitatively \redbf{modest}: the composition-adjusted gross lifetime return is \redbf{\pounds 41,000}, compared with \redbf{\pounds 45,000} in the unadjusted estimate.
...
\end{equation}
%
where $s$ indexes student subgroups, $\hat{\pi}_{1s}$ is the subgroup-specific first-stage effect, $N_s$ is the number of students in subgroup $s$ per cohort, $\hat{\beta}_{1s}^{\text{private}}$ is the net private lifetime earnings gain, $\hat{\beta}_{1s}^{f}$ is the fiscal return per student induced to take economics, and $r$ is the discount rate.\footnote{We value all quantities in 2025 prices. The lifetime earnings estimates from Section~\ref{sec:Impact} are uprated from their original price base using cumulative growth in the ONS Average Weekly Earnings index.} On the cost side, $C^{\text{sign-on}}$ is the government's share of initial teacher training costs (approximately \redbf{\pounds 25,000}, primarily through student loan write-offs), and $C^{\text{annual}}$ is the teacher's annual salary plus employer pension and social security costs. The annual salary is \redbf{\pounds 45,000} (the median classroom teacher salary from the DfE School Workforce Census, uprated to 2025 prices), and additional employer costs add approximately 42\% (the Teachers' Pension Scheme employer contribution of 28.68\% plus employer National Insurance contributions of 15\% above a minimum threshold), giving $C^{\text{annual}} = \text{\redbf{\pounds 64,000}}$. We assume an expected tenure of $T = \text{\redbf{10}}$ years, based on retention data from the NFER Teacher Labour Market Report \citep{worth2024teacher}.

The composition-adjusted average exchequer return per student from universal provision is \redbf{\pounds 14,000}. With a first-stage takeup effect of 5.7 percentage points and an average cohort size of \redbf{40} in never-offer schools, each school cohort generates an average exchequer return of approximately \redbf{\pounds 32,000}. Over a \redbf{10-year} teaching career, the total discounted exchequer return is \redbf{\pounds 275,000}. This covers roughly \redbf{half} of the \redbf{\pounds 575,000} cost of a teacher. Adding the private gains, the MVPF is \redbf{2.0}: each pound of net public cost generates around \redbf{two} pounds of private lifetime benefit (Table~\ref{tab:mvpf} Panel~A). Among the 133 historical US policies evaluated by \citet{hendren2020mvpf}, an MVPF of \redbf{2.0} sits well above the bulk of spending on adults, though below the direct investments in children that fully pay for themselves. \redbf{[These figures price only the degree-choice channel, which Panel B of Table~\ref{tab:degree_channel} shows is the smaller part of the age-28 return; if the full return persisted over the lifecycle, the exchequer return alone would exceed the teacher cost and the MVPF would be infinite. Revisit when the external model's figures arrive.]}

The UK government currently offers training bursaries of up to \pounds 30,000 for shortage subjects, principally in science and mathematics, recognising the high value of expanding teacher supply in those fields. Economics does not receive such a bursary. Given the returns documented here, a comparable incentive would be justified: with a \pounds 30,000 training bursary, the MVPF would still be around \redbf{1.8},
and the case for the incentive would stand.

\input{tables/ch4/table5_mvpf.tex}

Table~\ref{tab:mvpf} Panel~B shows who would benefit. Schools that do not currently offer economics disproportionately serve students from economically disadvantaged backgrounds. As well as being fiscally beneficial, expansion of access therefore also diversifies the economics pipeline. Universal access would increase the share of women from 28\% to \redbf{30\%}, the share from FSM backgrounds from \redbf{4\%} to \redbf{5\%}, and the share from outside the London and the South East from \redbf{45\%} to \redbf{49\%}.
...
Whether a school employs an economics teacher determines whether its students can study A-level economics, and this in turn has large consequences for degree choice and lifetime earnings. Between 6\% and 75\% of the gaps in A-level economics take-up across demographic groups are explained by differences in school-level availability alone. Students induced to take A-level economics by within-school changes in availability earn around 14\% more at age 28, corresponding to a predicted lifetime return of at least \redbf{\pounds 45,000}. The exchequer return to hiring an additional economics teacher covers roughly \redbf{half} of the cost, and the MVPF is \redbf{2.0}: each pound of net public cost generates around \redbf{two} pounds of private lifetime gain.

Expanding economics teacher supply would generate returns of around \redbf{twice} its net cost, even with a training bursary comparable to those offered for science and mathematics. Because schools that do not currently offer economics disproportionately serve disadvantaged students, the same policy also broadens the pipeline: the share of femlae economics undergraduates would rise from 28\% to \redbf{30\%}, and the share from poorer socioeconomic backgrounds (FSM) would rise from \redbf{4\%} to \redbf{5\%}. Supply-side expansion is a necessary precondition for the demand-side interventions that have dominated the diversity discussion: exposing students to role models or redesigning introductory courses cannot help students who cannot access the subject.
