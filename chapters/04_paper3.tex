...
The age-28 estimates capture only the early-career return to A-level economics. To assess lifetime effects we combine our estimates with published estimates of the lifetime returns to undergraduate degrees \citep{britton2026lifetime}, which measure, for each degree subject, the gain relative to not attending university: gross, net of income tax, National Insurance and Plan~5 student loan repayments, and for the exchequer, in 2025/26 prices and discounted to age 18 at Green Book rates.

Taking A-level economics operates on two margins: it raises university attendance by 14.0 percentage points (Appendix Table~\ref{tab:other_outcomes}) and economics degree take-up by 15.2 percentage points (Table~\ref{tab:degree_channel}). Valuing the attendance margin at the average net lifetime return of \redbf{\pounds 100,000} and the switching margin at the gap between the economics return of \redbf{over \pounds 400,000} and the returns to the subjects compliers would otherwise study \redbf{(around \pounds 75,000)} gives an expected net individual return of approximately \redbf{\pounds 60,000} per induced student. An alternative calculation, which instead assumes the 14\% age-28 earnings premium persists over the lifecycle, gives a similar magnitude. Applying the same components to the gross and exchequer schedules implies a gross return of roughly \redbf{\pounds 110,000} and an exchequer return of roughly \redbf{\pounds 45,000} per induced student.
...
Two caveats apply. The published returns are subject-level averages estimated under selection on observables, so the calculation averages over heterogeneity within subjects and inherits that identification. And it prices only the degree-choice channel: Panel~B of Table~\ref{tab:degree_channel} shows that much of the age-28 return arises among compliers who do not change degree subject, so these figures understate the full lifetime effect.
...
The composition-adjusted average exchequer return per student from universal provision is \redbf{\pounds 41,000}. With a first-stage takeup effect of 5.7 percentage points and an average cohort size of \redbf{40} in never-offer schools, each school cohort generates an average exchequer return of approximately \redbf{\pounds 94,000}. Over a \redbf{10-year} teaching career, the total discounted exchequer return is \redbf{\pounds 810,000}. Since this exceeds the \redbf{\pounds 575,000} cost of a teacher, the MVPF is infinite: the policy more than pays for itself, with a fiscal surplus of \redbf{\pounds 235,000} per teacher (Table~\ref{tab:mvpf} Panel~A). Among the 133 historical US policies evaluated by \citet{hendren2020mvpf}, an infinite MVPF places this intervention alongside the highest-return investments in children's education and health. This calculation prices only the degree-choice channel; Panel~B of Table~\ref{tab:degree_channel} shows that much of the age-28 return arises outside it, so the fiscal return is understated and the conclusion is conservative.

The UK government currently offers training bursaries of up to \pounds 30,000 for shortage subjects, principally in science and mathematics, recognising the high value of expanding teacher supply in those fields. Economics does not receive such a bursary. Given the returns documented here, a comparable incentive would be justified: with a \pounds 30,000 training bursary, the fiscal surplus per teacher would still be \redbf{\pounds 205,000},
