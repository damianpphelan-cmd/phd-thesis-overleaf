...
To put a scale on the misallocation, we apply the access gaps in Table~\ref{tab:access_takeup} to current cohort sizes. Equalising access across groups would generate approximately 4,000 additional A-level economics students per cohort: an increase of 27\% relative to current numbers. The demographic composition of A-level economists would shift correspondingly, with the share from FSM backgrounds rising from 6.4\% to 6.6\%, and the share from outside London and the South East
from 52\% to 58\%.
