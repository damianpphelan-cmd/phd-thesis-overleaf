...
Across all comparisons, the access component explains between 6\% and 82\% of the take-up gap. This range provides a transparent lower bound on the role of supply constraints: it takes no stand on the causal effect of access on take-up (which may be larger than the mechanical contribution if access also affects preferences through peer effects or information), and it does not rely on any regression specification. The causal estimate follows in Section~\ref{sec:Impact}.
