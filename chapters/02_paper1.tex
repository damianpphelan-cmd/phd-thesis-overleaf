...
not fitted on. Before fitting, identifying details are removed from the text. The school's intake is also taken out of the score the model is asked to predict. Schools with similar intakes tend to receive similar culture scores, and a report often reveals what a school's intake is, so without this step a model could score well by recognising the intake rather than by reading anything about the culture. The model is therefore asked to predict only the part of a school's score that its intake does not explain. 
