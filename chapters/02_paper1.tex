...
A simple count shows the gap. Sort the schools into a warmer half and a colder half, once by 
the heads' ratings and once by the observers' scores. If the two measures agreed perfectly, 
every school would fall in the same half both times. In fact \HalfMatchW{} of the 103 do.

Two benchmarks put that number in context. The first is chance. If the heads' ratings had nothing to do with the observers', half the schools would still land in the same half, about \HalfLuck{} of the 103. The second is the most the agreement could be. Even if heads and observers were measuring exactly the same thing, measurement error would move some schools across the middle, and given how noisy the two measures are, the agreement would reach about \HalfCeilingW{} of the 103. The actual figure, \HalfMatchW{}, is close to the chance figure and well below the ceiling: on warmth, the heads' ratings agree with the observers' scores no more than chance would produce. The heads' scale is coarse, so several schools sit exactly at the middle and must be counted into one half or the other. Counting them the other way gives \HalfMatchWAlt{}, and the conclusion is the same.

Strictness is different. There the agreement is \HalfMatchS{} of 103, above the chance benchmark and below the ceiling of \HalfCeilingS{}. Heads place their school's strictness with some accuracy, though not with all the accuracy the measures would allow. The disagreement is concentrated in warmth, and noise does not account for it: correcting the warmth correlation for the noise in both measures raises it only to $0.42$.

\subsection{What the public text contains}

This section shows three things. First, the public text does contain culture, but only in one place: every reading that passes the tests comes from an inspection report, and the documents schools write about themselves carry no signal. Second, within the reports the two dimensions are read by different methods: the prediction model reads warmth, the marking scheme reads strictness, and teaching is read by both. Third, the readings the chapter goes on to use pass the three checks set out in the measurement section.
...
scheme does better than the prediction model, although the gap between the two is small enough to 
be chance 
at this sample size ($z = \StrictDiffZ$, $p = \StrictDiffP$). The numbers alone therefore cannot 
separate the two methods on strictness. The three checks set out in the measurement section can, and the scheme's reading passes all three: it gives the same band on a second run, a different language model applying the same page reads the school almost identically, and its bands do not track document length.

The retest asks whether the language model gives the same answer twice. Each scheme was run twice on the same documents. Across all fourteen readings, the two runs give the same band \GridRetestExact{} of the time. The two strictness schemes, one reading the inspection reports and one reading the behaviour policies, gave every school the same band on both runs. The weakest agreement between runs is the policy warmth scheme at 83\%, and that is a reading the chapter does not use.
...
the same rubric should find the same thing. For strictness, the second model reads the 
enacted scores at $\RubricSFourO$, close to the primary model's $\RubricSOfstedRaw$, so the 
scheme is what determines the band, not the model. The warmth and teaching schemes fail the same check. Applying the same pages, the second model reads essentially nothing ($\RubricWFourO$ for warmth, $\RubricTFourO$ for teaching), so what the primary model found there came from the model rather than the scheme. 
...
Demographic adjustment changes the strictness reading little: the scheme scores $\RubricSOfstedRaw$ raw and $\RubricSOfstedDec$ with everything demographics could explain removed. The warmth scheme fails entirely: it reads the reports at $\RubricWOfstedDec$ and 
...
Across the \PVPredWarmthN{} schools, predicted warmth and parent-reported warmth correlate at $\PVPredWarmth$; \cref{fig:p1_pv} plots the two measures against each other. That agreement would be worthless if the model had been 
