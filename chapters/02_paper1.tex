...
rather than concentrated in quotable lines. The same is true of observed teaching: on the same held-out basis, the model's predictions correlate with the observers' teaching scores at $\PredTOfsted$.

The second finding is that enacted strictness must be read differently.
The prediction model reads it only weakly. The one-page mark scheme does better: its bands correlate with enacted strictness at $\RubricSOfstedDec$. The
