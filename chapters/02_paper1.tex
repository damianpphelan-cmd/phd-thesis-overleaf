...
The two enacted scores are correlated at $r = \CorrWarmthStrictnessEnacted$ across the \NTierOne{} 
visited schools, and the espoused 
pair at $\CorrWarmthStrictnessEspoused$. Read at face value, the enacted figure says that warm 
schools are usually strict ones. 
It should not be read that way, because part of it is produced by the measurement itself: the 
same observers scored both dimensions 
at the same school on the same day, and a good impression lifts both ratings at once. The lessons 
that two researchers rated 
independently show how much this explains. If the correlation lived inside each observer's head, 
an observer's warmth rating would 
predict their own strictness rating much better than their colleague's. That is not the 
case: \CorrWSSameObs{} against \CorrWSCrossObs{}. 
So the inflation comes from the shared visit rather than from individual judgement, since 
both observers watched the same lessons at 
the same school on the same day. Removing measurement noise from the cross-observer figure puts 
the correlation between the dimensions 
themselves at about \CorrWSTrue{}. That, rather than $\CorrWarmthStrictnessEnacted$, is the 
better estimate of how strongly warmth and 
