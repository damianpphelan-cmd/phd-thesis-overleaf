...
the same check. Applying the same marking schemes, the second model reads essentially 
...
Demographic adjustment is separate from the three checks, and it is applied to the marking-scheme readings in the same way as to the prediction models'; \cref{sec:p1_app_prediction} gives the specification. The adjustment changes the strictness reading little: the scheme scores $\RubricSOfstedRaw$ raw and $\RubricSOfstedDec$ adjusted. The warmth scheme fails entirely: it reads the reports at $\RubricWOfstedDec$ and 
...
framework boilerplate. The deconfounded specification regresses each outcome on the school's free-school-meals share, share of pupils with English as an additional language, prior attainment and report age, and takes the residual as the target. The marking-scheme readings are adjusted the same way: the bands are left as awarded, and the adjusted figure is their correlation with this residual.
