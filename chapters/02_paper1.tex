...
Two corrections give a sense of how much the error takes off. The first corrects the enacted--espoused correlations for the error in both scores at once. They rise to about \DisSplitW{} for warmth and \DisSplitS{} for strictness. Two error-free readings of the same school would correlate near one, so even after the correction the head and the observers are far from describing the same school. The second corrects the marking scheme's strictness correlation with the enacted score, $\RubricSOfstedRaw$, for the enacted score's error alone, and it rises to about $0.36$. In both cases the corrected number leads to the same verdict as the raw one, so the uncorrected figures are the ones reported throughout.
