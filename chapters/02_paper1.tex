...
The second method uses a mark scheme approach. A large language model is provided with a one-page written mark scheme for each dimension and
...
Demographic adjustment is separate from the three checks, and it is applied to the mark-scheme readings in the same way as to the prediction models'; \cref{sec:p1_app_prediction} gives the specification. The adjustment changes the strictness reading little: the scheme scores $\RubricSOfstedRaw$ raw and $\RubricSOfstedDec$ adjusted. The warmth scheme fails entirely: it reads the reports at $\RubricWOfstedDec$ and 
...
The second result is that the agreement is not the inspection grade in disguise. If the grade were all the model reads, then 
...
direct observation: prediction models for warmth and teaching, and a one-page mark scheme 
