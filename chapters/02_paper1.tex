...
Direct measurement shows two things. The first is that warmth and strictness are separate dimensions of a school. A school can be warm without being orderly, and orderly without being warm, and the visited sample contains both kinds: \cref{fig:p1_quadrants} plots the schools on the two dimensions and every quadrant is occupied. The two dimensions are related but distinct: once observer effects and measurement noise are removed, they correlate at \CorrWSTrue{}. The enacted scores are reliable enough to use, and their limits are measured rather than assumed; \cref{tab:score_reliability} reports the reliabilities.
...
The second is that what heads say and what observers see are different measures. The interview and visit scores are correlated at only $\GoldWarmthSplit$ for warmth ($p = \GoldWarmthSplitP$) and 
