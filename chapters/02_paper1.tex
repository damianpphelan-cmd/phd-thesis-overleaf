...
than chance but short of what two noisy readings of the same school would give. Fourth, schools' own documents track their heads on strictness and carry no signal about warmth.
\end{chapterabstract}
...
In this study, I define warmth as
...
used consistently across the lessons of the day. I define strictness as the school's
...
enforced (demandingness), predicts the level of disorder in the school even after accounting for the demographics of its pupils. All of these studies rely on surveys of pupils, parents or staff, which may be affected by perceptions, and do not apply the definitions of demandingness and responsiveness consistently. 
%use slight variations on the demandingness and responsiveness language. 
In contrast, this paper measures warmth and strictness consistently across schools and through direct observations and interviews with headteachers, who oversee and manage the policies underlying actual school culture. 
...
together in practice is an empirical question.
...
Warmth and strictness are not directly observed for all schools in England, but only for the 103 schools in which I conducted school observations and structured headteacher interviews. Building on the evidence generated from these school visits, this paper proposes a new measure of warmth and strictness that can be built from public
text that is available for each school.
For every school, there exist a report written by an external inspector, a behaviour policy written by the
school, and a website written by the school. I exploit a set of 103 schools in which school-visit-based measures are available to estimate a model that can accurately predict these measures from the publicly available text alone. This model, then, can be applied to all schools in the country to build predicted warmth and strictness measures. 

%Since in 303 schools (the 103 visited schools, plus a further 200) I also conducted structured headteacher interviews, in this second sample of schools I ... unclear how you use this sample for now.
...
From the observation and structured interview data, I construct a gold-standard measure of 
warmth and strictness. Because the interview and the visit disagree substantially about the same school, each gold standard score, one for warmth and one for strictness, is in fact two-dimensional: an \emph{enacted}
score, from what the observers recorded, and an \emph{espoused} score, from
what the headteacher reported. 

I then examine whether these scores can be accurately predicted from a school's public text alone using
...
higher or lower enacted scores, and it is tested only on schools it was
not estimated on. Before estimation, identifying details are removed from the text,
and the part of each score that school demographics could explain is removed from the target, so the model is reading culture from the prose only. 

The second method uses a mark scheme approach. A large language model is provided with a one-page written mark scheme for each dimension, which I produced, and
asked to score each document on a 1 to 5 scale. The first method is ex-ante agnostic about which components of the text are associated with warmth or strictness; the second, in  contrast, scores the text according to how informative it is about school culture based on the marking scheme that I produced. Comparing the two 
...
carry a modest but real signal of enacted warmth. A prediction model
reading only the report's wording reproduces the enacted warmth of
...
visit are correlated at only $\GoldWarmthSplit$ for warmth, against $\GoldStrictnessSplit$ for strictness. A simple count says what a correlation that low means in practice. Sort the schools into a warmer and a colder half, once by the heads' ratings and once by the observers'. If the two measures agreed, most schools would fall in the same half both times. In fact the two sortings agree at \HalfMatchW{} of the 103 schools. Chance alone would agree at about \HalfLuck{}, and two noisy readings of the same quantity at about \HalfCeilingW{}. For strictness the agreement is \HalfMatchS{} of 103, against a ceiling of about \HalfCeilingS{} once measurement error is allowed for.
...
\begin{figure}[htbp]
  ...
  \caption{The warmth model and the strictness scheme against the enacted scores, after the demographic adjustment (the part of each enacted score that the four control variables do not explain). Left: the warmth prediction
  model's held-out predictions against enacted warmth (each school
  predicted by a model that never saw it). Right: the strictness
  mark scheme's bands against enacted strictness.}
...
The first result is the national agreement. The warmth prediction model was fitted on the visited schools alone. That leaves \PVPredWarmthN{} schools the model 
...
schools. For each school $s$, the model is fitted on every other school,
%
\begin{equation}
