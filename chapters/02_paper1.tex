...
separate the two methods on strictness. The three checks set out in the measurement section can: the retest, the change of language model, and the length check. The scheme's strictness reading passes all three, as the next paragraphs report.

The retest asks whether the language model gives the same answer twice. Each scheme was run twice on the same documents. Across all fourteen readings, the two runs give the same band \GridRetestExact{} of the time, and the report strictness scheme gave every school the same band on both runs. The weakest agreement, 83 per cent, is in a policy scheme whose reading the chapter does not use.
...
The length check asks whether the marking scheme bands track document length rather than the school. For the inspection-report schemes the worry is small: the strictness bands correlate with length at $\LenOfstedS$ and the warmth bands at $\LenOfstedW$, and removing the effect of length moves the strictness reading only from $+0.25$ to $\RubricSOfstedLenP$. The website schemes' bands track length closely, one more reason their readings are not used.
