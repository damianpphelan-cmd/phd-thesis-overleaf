...
not fitted on. Before fitting, identifying details are removed from the text, and the scores are adjusted. A school's intake --- the prior attainment of its pupils, and the shares who are eligible for free school meals or who speak English as an additional language --- predicts part of its culture score on its own. So each school's score is replaced by the difference between the score the observers gave it and the score a school with its intake typically receives. A school that is warmer than its intake would suggest has a positive value, and a colder one has a negative value. That difference is what the model is asked to predict. Without this step a model could score well by recognising a school's intake from its report rather than by reading anything about its culture. 
...
culture shows up in text; the second assumes it is there and scores it. Each reading is then judged on whether it agrees with what the observers recorded, and only the readings that pass their tests are applied to the national population. The two methods face different tests, because they can fail differently. A prediction model might succeed by recognising a school rather than by reading it, so the whole procedure is run again on scores shuffled between schools, and a reading has to beat what that shuffling produces. A mark scheme might succeed because of the language model applying it, or because it is tracking how long a document is rather than what it says, so its reading has to survive a second scoring run, a different language model, and the removal of any effect of length. Both kinds of reading must also hold up once allowance is made for the number of readings tried.
