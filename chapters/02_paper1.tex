...
A visit lasted a school day. \DaysTwoPlusRaters{} of the \NTierOne{}
visits were made by a team of two or three researchers, and
\DaysOneRater{} by a single researcher. What
single-researcher visits mean for the scores is examined in the
reliability section below and in the next chapter's robustness checks. The
school was asked in advance to arrange four to six
...
outside-of-lessons form was completed by two or more researchers on
\DaysTwoPlusRaters{} of the \NTierOne{} visits, and by one researcher on
the other \DaysOneRater{}. A researcher also left an item blank when they
did not see what it asks about, so for any one item the number of visits
with two ratings to compare is smaller than \DaysTwoPlusRaters{}.
...
One caveat qualifies the warmth result from the prediction models. The espoused warmth score is built from just three interview
...
Models built on sentence tags alone (topic and polarity) predicted every score worse than models built on the words. A second variant multiplied each word count by plus one in a positive sentence and minus one in a negative sentence. This raised the correlation for warmth, strictness and teaching alike. The words responsible were clerical ones from the report format. A feature that helps all three dimensions at once, and does so through format words, is picking up how the report was written rather than what the school is like. The chapter's rule discards such features, so the variant was dropped.
