...
than chance but short of what two noisy readings of the same school would give. Fourth, schools' own documents match what their heads say about strictness and carry no signal about warmth.
\end{chapterabstract}
...
not fitted on. Two things are done before fitting. First, identifying details are removed from the text. Second, the scores are adjusted for intake, because schools with similar intakes tend to receive similar culture scores. Two schools count as having similar intakes when their pupils have similar prior attainment, and similar shares are eligible for free school meals or speak English as an additional language. Each school's score is then measured against the average score of schools like it. A school that is warmer than those schools gets a positive number, and a colder one gets a negative number. It is that number the model is asked to predict. Without this step a model could score well by recognising a school's intake from its report rather than by reading anything about its culture. 
