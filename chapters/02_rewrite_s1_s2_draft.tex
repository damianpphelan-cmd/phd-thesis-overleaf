...
and met. The two are the axes of the authoritative-parenting framework
transposed to institutions, and both are plausibly inputs to learning. The
difficulty is that neither is recorded in any administrative dataset. What is
...
warmth, and the text-only model's leave-one-out correlation is
$\RidgeBWarmth$. Second, the enacted and espoused measures diverge in a
systematic and dimension-specific way. Headteachers rate their schools half a
point warmer than observers record and about as strict; the interview and
