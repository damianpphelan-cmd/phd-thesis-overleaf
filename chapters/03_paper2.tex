...
essentially unchanged when the inspection grade is held constant and on
reports that predate the outcome years.
\end{chapterabstract}
...
or 14. Finally, the strictness association is found again across the national
...
those examinations. The remaining components, EBaC and Open, are partly
...
observed strictness at $\RubricSOfstedRaw$. The previous chapter
explains why the two dimensions need different instruments: warmth lives
...
\begin{figure}[htbp]
  ...
  one-page marking scheme (3,324 schools). All five bands are used;
...
and insignificant ($\BetaNationalAgeInt$ per year, $p =
\PNationalAgeInt$, measured with the earlier strictness instrument;
\pending{re-estimate the age interaction with the marking-scheme
bands}), which is the behaviour a persistent signal would produce. Because the 2019 grade is held constant, the comparison is between
...
rather than assumed away. Secondly, the strictness association appears again across the
...
the sum of these two components: it also counts the EBaC and Open slots,
and it is there that the strictness association is clearest (EBaC $0.138$,
...
controlling for EBacc entry attenuates the association by 11 to
14~per~cent nationally, and by a similar 11~per~cent in the visited
sample. What only the national data can show is the first stage, since at
...
It should be added that the national panel, with thirty times the sample,
does detect a small positive interaction ($0.016$, $p < 0.001$ with the
grade control; \cref{tab:typology}). That interaction is an order of
magnitude smaller than the strictness main effect, although about half the
size of the national warmth one, and its sign is the opposite of a
trade-off, so the additive description remains a fair summary; the
exact statement is that the visited sample shows no interaction and the
national panel a small positive one. It should be noted that the national quadrant
analysis predates the present instruments and used the earlier
report-derived scores, so the national interaction is read as suggestive
rather than as a test on the visited constructs. \pending{re-run the
national typology on the new predicted scores} Breaking
...
The warmth score is an estimated quantity, so its regression was
bootstrapped over the training fit: the 95 per cent interval is
[$\NatWBootLo$, $\NatWBootHi$], comfortably away from zero. Excluding the
...
carry, and not against one another. The two model legs behave like clean,
faint copies of the visited estimates: a proxy capturing about a third of
true warmth should show about a third of the visited warmth effect, and
$\NatW$ is almost exactly that; teaching matches its own arithmetic the
same way. The strictness leg sits roughly twice its equivalent benchmark.
...
It should also be noted that the national standard errors treat schools
as independent; schools in the same academy trust share practices, and no
trust identifier is attached to the dataset, so the reported precision is
likely somewhat overstated, although the coefficients would clearly
survive any plausible clustering.
...
\pending{Re-run the entry-rate analysis (EBacc, humanities, languages)
with the marking-scheme bands in place of the retired strictness score;
the earlier instrument's version attenuated the EBaC association by only
11--14 per cent, indicating progress within subjects rather than entry
steering.}
...
register --- carry their own associations with progress
(\cref{tab:identity}). Schools whose websites espouse a traditional
...
significant ($0.113$, $p = 0.33$). Restricting the sample to the 64 schools in
...
on the overall measure (\cref{tab:p8_proxy}); the contemporaneous
strictness coefficient is if anything larger, which sits well with the
timing argument. Restricting the sample to the 22 schools where the headteacher named in
...
outside the classroom, in corridors, at break and at the gate, which is
where schools currently direct the least observation and training
effort. Both implications are conditional on the association being at
...
measured by observation in a hundred ordinary schools, with the
strictness half extended to the national population through a text-based
measure, rather than in a case-study sample. To the charter and academy literature, they say that the
...
strongly in the EBaC component and warmth in the Open component, is
...
EBacc entry rate to the EBaC specification reduces the strictness
...
\cref{tab:national_strictness_full} the earlier strictness
instrument's national extension on all five outcomes
(\pending{regenerate the five-outcome national table from the
marking-scheme bands}); the EBaC and Open coefficients are of the same
size as the others.
