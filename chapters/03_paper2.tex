...
first is that both dimensions predict progress. A one standard deviation
...
more progress and are scored lower on warmth; once intake is held constant,
it is the harder of the two to attribute to an unmeasured confounder. The
...
the school removes the warmth association, while a strictness score read
from inspection reports shows the same association across
\NNationalStrictness{} schools ($\BetaNationalStrictness$), essentially
unchanged when the inspection grade is held constant and on reports that
predate the outcome years.
\end{chapterabstract}
...
There are three main results. First, both dimensions predict progress.
...
constant, and is then the harder of the two to explain away, while the
...
association entirely, and the strictness score read from inspection reports
shows the same association across the national population with the
...
distance from a school near the tenth percentile on both dimensions to one
near the ninetieth, is associated with about half a grade per subject per
...
entry-rate analysis suggests the entry channel itself is small, since
controlling for EBacc entry attenuates the association by only 11 to
14~per~cent, but that analysis is national; in the visited sample alone,
strictness does not significantly predict EBacc entry. The full component
...
magnitude smaller than either main effect and its sign is the opposite of
a trade-off, so the additive description remains the right summary; the
...
carry a coefficient of roughly $0.4$ times the visited one; the national
coefficient is larger than the visited one, so much of it rests on
variance in what reports say that is not observed strictness --- the
...
estimates are presented as associations. What the extension does not provide
...
has a coefficient of $0.025$, $p = 0.76$). Restricting the sample to the 64 schools in
...
two-fifths of a grade ahead. On these estimates, the schools that are strict but not warm sit no
higher than the schools that are neither; if any part of the association
is causal, an improvement drive that tightens behaviour systems while
letting relationships slip would forfeit about as much as it gains.
...
others of similar intake, and is then the harder of the two to attribute
to an unmeasured confounder, while the strictness association is present
in the raw data but is the more exposed to one. The two dimensions
...
school predicts nothing on warmth, and a strictness signal read from
inspection reports shows the same association nationally, with the
inspection grade held constant and on reports that predate the outcome
years.
...
order of magnitude smaller than the main effects. \Cref{tab:gaps} examines
