...
the school removes the warmth association, while an independently produced strictness signal read from inspection
reports predicts progress across \NNationalStrictness{} schools
($\BetaNationalStrictness$), essentially unchanged when the inspection
grade is held constant and on reports that predate the outcome years.
\end{chapterabstract}
...
association entirely, and an independently produced strictness signal read from inspection
reports predicts progress across the national population with the
inspection grade held constant.
...
strictness score enters the national extension. That score is produced by
a language model that reads each school's most recent inspection report
with the graded judgements removed and every stated grade deleted, and
returns a strictness rating on a five-point scale under a fixed marking
scheme; the previous chapter describes its construction and testing in
full. Its correlation with observed strictness in the visited schools is
$\CorrOfstedStrictness$. The companion warmth score tracks the tone of
the report's verdict rather than observed warmth, which is why only the
strictness leg is carried forward.
...
\begin{figure}[htbp]
  ...
  \caption{Distributions of the inspection-report warmth (left) and strictness (right) scores
    ...
    not used as the warmth leg of that extension, for the reasons given in
    the previous chapter, and appears only in the exploratory tables of the
    appendix and as the warmth leg of the national quadrant analysis in
    \cref{tab:typology}.}
...
the interaction of the strictness score with report age is
$\BetaNationalAgeInt$ per year ($p = \PNationalAgeInt$), which is the
behaviour a persistent signal would produce. Because the 2019 grade is held constant, the comparison is between
...
national panel a small positive one. It should be noted that the warmth
leg of the national panel is the inspection-report warmth score, which the
previous chapter found tracks the tone of the verdict, so the national
interaction is read as suggestive rather than as a test on the visited
constructs. Breaking
...
$0.005$. This result has two implications. The first concerns
...
estimated on every school with a report and an outcome, so the association
is not a property of the schools that agreed to take part. The two
coefficients should not, however, be read as the same estimand. The report
score tracks observed strictness at $r = \CorrOfstedStrictness$, and a
score that were nothing but a noisy stand-in for observed strictness would
carry a coefficient of roughly $0.4$ times the visited strictness-only
coefficient of $0.199$, which is about $0.08$; the national coefficient
sits well above that benchmark, so much of it rests on variance in what
reports say that is not observed strictness --- the
breadth of what inspectors describe, and whatever of their knowledge of
the school reaches the prose. The right statement is that an independently
produced strictness signal predicts progress at national scale, robust to
the grade control and to report timing, and not that the visited
coefficient has been reproduced. A further check concerns timing:
\ShareNationalPre~per~cent of the reports behind the national score were
published before September 2022, before either outcome cohort's results
existed, and on that subsample alone the coefficient is
$\BetaNationalPre$ ($n = \NNationalPre$), essentially the full-sample
value. The reports that could have been written in knowledge of the
outcomes are therefore not driving the association. What conditioning on
the grade cannot do is remove outcome information that sits in the
report's text but not in its verdict: inspectors see a school's results,
and two schools with the same grade may be described differently partly
because of them. The pre-window subsample meets the sharpest version of
that concern, since those reports predate the results entirely, but a halo
from earlier results remains possible, and it is one of the reasons the
estimates are presented as associations. It should also be noted that the
national standard errors treat schools as independent; schools in the
same academy trust share practices, and no trust identifier is attached
to the dataset, so the reported precision is likely somewhat overstated,
although the coefficient would clearly survive any plausible
clustering. What the extension does not provide
is the same reassurance for warmth, since no text source contains observed
warmth (the inspection-report warmth score carries a small association
with progress in \cref{tab:llm_p8_matrix}, but the previous chapter found
that it tracks the tone of the verdict rather than observed warmth, so it
is not read as a warmth replication), and nor does it provide
identification, since a text-derived measure
...
is negatively associated with progress, though after the false-discovery
correction the association is significant only for the Open component, and the website
warmth and strictness scores are not associated with it at all.

A further question is whether the strictness association runs through the
curriculum rather than through progress within it. Stricter schools do steer
pupils towards the academic pathway: nationally, the strictness score
predicts entry to the EBacc suite, to the humanities and to languages with
the inspection grade held constant. However, controlling for EBacc entry
reduces the strictness association with the EBaC component by only 11 to
14~per~cent (\cref{tab:entry_rates}). This means that most of the
association reflects progress within subjects, and not which subjects pupils
are entered for.
...
school predicts nothing on warmth, and an independently produced strictness signal read from inspection
reports predicts progress nationally, with the inspection grade held
constant and on reports that predate the outcome years.
