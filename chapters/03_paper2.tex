...
or 14. And the strictness result is reproduced across the national
...
$|0.3|$ (\cref{tab:score_controls_corr}). This means that the culture
measures are not close proxies for intake or school type, and that the
coefficients below are not composition effects in disguise.
...
not ruled out by the timing. What the design relies on instead is that
culture is persistent, so that what was observed in 2024/25 is a fair
reading of what the 2022/23 and 2023/24 cohorts experienced. The appendix
...
terms of staff and families, is partly absorbed. And because the sample
excludes schools that enrol at 13 or 14, no school is credited with progress
it did not house.
...
rather than assumed away. And the strictness result reproduces across the
...
quarter of schools by free-school-meal share ($0.62$). Two standard
deviations on both dimensions together, which is the distance from a cold
and disorderly school to a warm and orderly one within the visited range, is
associated with about half a grade per subject per pupil. That is the size
of the difference that the performance tables treat as separating a
well-above-average school from an average one.

Entered on its own, each dimension carries a coefficient of about $0.2$
...
carries substantial information that the other does not. This is the
...
At the level of the components, warmth carries both subjects (English
...
There are two possible readings of that result, and the data do not choose
between them. The first is that culture and instruction are distinct
predictors of progress. On this reading, what a school is like between and
around lessons, and how orderly its lessons are, matter over and above how
well the lessons are taught, so that the culture association is not the
teaching-quality association under another name. The second is that the
teaching score is the noisiest of the three visit measures, being read from
ten items on which the observers agree least, and that its signal is
absorbed by the two culture scores with which it is correlated, so that the
null result on teaching understates what instruction contributes. Three
facts favour the first reading without settling the matter. The teaching
score on its own predicts progress well, so it is not merely noise. The
culture coefficients do not move at all when it enters, whereas absorption
would show up as movement in both directions. And the culture association
runs partly through out-of-lesson warmth, which no measure of classroom
teaching could carry. What would separate the two readings is a measure of
teaching quality that is independent of the visit, and none exists for these
schools. The claim I make is therefore the narrower one: conditioning on
observed teaching quality does not account for the culture association.
...
both because both help, not because of a combination effect, and a school
that is strong on one dimension gains from the other regardless. Breaking
...
($0.120$, $p = 0.018$). The measurement asymmetry of the previous chapter
reappears here as a predictive one. What a headteacher says about warmth
carries no information about progress, and what they say about behaviour
systems carries some. This is the pattern that the previous chapter's
...
the full control set without the grade it carries $\BetaNationalStrictness$
($p < 0.001$, $R^2 = 0.52$; \cref{tab:national_strictness}). With the 2019
grade added ($n = 2{,}746$) it carries $0.140$, and no component estimate
moves by more than $0.005$. Two things follow from this. The first concerns
the score itself. A score read from an inspection report might be thought to
be a proxy for the inspectorate's overall approval, so that the association
is really the grade's association under another name. But the coefficient is
untouched by conditioning on the grade, and a proxy for the grade would lose
most of its coefficient when the grade entered. What the score carries is
what the report says about conduct and enforcement, over and above the
verdict, and that is what predicts progress. The second concerns the visited
...
of the schools that said yes. What the extension does not buy is the same
reassurance for warmth, since no text source carries observed warmth, nor
does it buy identification, since a text-derived measure inherits the same
omitted-variable concern as an observed one. Two further text-derived
...
0.12$).
The enacted scores might carry an artefact of the visits themselves. About
...
carries $0.03$, $p = 0.76$). Restricting the sample to the 64 schools in
...
way (\cref{tab:continuity_robustness}).
...
coefficient. The behaviour debate treats discipline as the active ingredient
of school improvement and relational culture as secondary. These estimates
...
behaviour systems and lets relationships go is giving back what it gains.
The second implication is that the association runs through relational
quality outside lessons and orderly conduct within them. This locates the
relational work that matters outside the classroom, in corridors, at break
and at the gate, which is where observation and training effort are least
concentrated. Both implications are conditional on the association being at
...
two halves of the practice bundle, structure and support, each carry weight
when they are measured apart, and that neither is the whole of the effect.
And to the authoritative-school literature, they supply the
...
be ruled out with cross-sectional data. The visited sample is small and leans
towards stronger schools, culture was measured after the outcome window for
most schools, and one visit is a noisy reading of a school. A longitudinal
design, with culture observed before the outcome cohort arrived and again
afterwards, would speak to the direction of the association, and a design
that observed culture changing within schools, as headteachers changed or
behaviour systems were introduced, would speak to its cause. Within the
