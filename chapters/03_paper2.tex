...
signal (which itself has an ICC of $0.91$ for the KS2 scaled score) is
retained in the dependent variable and inflates between-school variance
relative to within-school variance. The higher Attainment~8 ICCs therefore do
not indicate that raw attainment is more stable than value-added in any
useful sense. They reflect the composition of the variance rather than the
reliability of the school-level signal.
...
full-data schools, continuity could be determined for only 29, of which 22
were unchanged and 7 had changed. Given the small $n$, the coefficients in
the continuity subsample are imprecisely estimated (the standard errors are
roughly four times larger than in the full sample), and neither warmth nor
strictness is statistically significant in any component. The strictness
point estimates are in the same direction as the full-sample estimates, and
in every component they are larger ($\hat{\beta}_S$ ranges from 0.27 to
0.33), while the warmth estimates are noisy and should not be interpreted
directionally. This check does not provide evidence against the primary
results, but the subsample is too small to detect effects of the size found
