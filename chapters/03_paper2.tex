...
a coefficient of $0.015$ and leaves both culture coefficients essentially unchanged. The
...
above the rest, and in the visited sample the interaction between the two
is zero. Replacing the
...
reproduces the strictness result across \NNationalStrictness{} schools
($\BetaNationalStrictness$), essentially unchanged when the inspection
grade is held constant.
\end{chapterabstract}
...
leaves both culture coefficients essentially unchanged, and the teaching coefficient
...
interaction between the two dimensions is zero in the visited sample. In
other words, the
...
what was observed in 2023/24 and 2024/25 is a reasonable measure of what the 2022/23
...
same time as the fieldwork gives coefficients within $0.03$ of the primary
...
deviation of school Progress~8 nationally. It is also about a quarter of the gap in mean
...
culture coefficients barely move when it enters, whereas if
...
does detect a small positive interaction ($0.016$, $p < 0.001$;
\cref{tab:typology}). That interaction is an order of magnitude smaller
than either main effect and its sign is the opposite of a trade-off, so
the additive description remains the right summary; the exact statement is
that the visited sample shows no interaction and the national panel a
small positive one. Breaking
...
of the schools that agreed to take part. A further check concerns timing:
...
as the fieldwork, gives coefficients within $0.03$ of their real-Progress~8
counterparts (\cref{tab:p8_proxy}). Restricting the sample to the 22 schools where the headteacher named in
...
standard errors four to six times the full-sample ones, and should not be
...
compares selection on unobservables with selection on the full control set.
Driving the warmth coefficient to zero would require unobservables
\OsterDeltaW{} times as important as the controls, against a conventional
benchmark of one, and warmth retains a coefficient of \OsterBetaEqualW{}
when unobservables are assumed to matter exactly as much as the controls.
It should be noted that for warmth the observed controls raise the
coefficient rather than lower it, so the equal-selection convention is not
a simple continuation of the observed movement; the bound is reported on
the standard convention and remains clearly positive.
The strictness estimate is less robust on this measure. Its $\delta$ of
\OsterDeltaS{} sits just above the benchmark, and its coefficient under the
equal-selection assumption is close to zero, because the controls already
absorb part of the raw association and the extrapolation continues that
movement. The second measure is the E-value. A confounder would need an
...
respectively, and both are stable across every treatment of the controls.
...
progress; it does not, and the apparently significant gap terms are shown in the final columns
...
correlation exceeds $|r| = 0.29$. The largest entries among the enacted
scores are between
...
coverage as by turnover. In this subsample the standard errors are four to six
