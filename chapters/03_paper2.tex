...
the school removes the warmth association. Finally, all three dimensions
extend to national scale through instruments validated in the previous
chapter: across \NNationalStrictness{} schools, text-predicted warmth
carries $\NatW$ per standard deviation, text-predicted teaching $\NatT$,
and strictness read by a one-page marking scheme $\NatS$ --- each
essentially unchanged when the inspection grade is held constant and on reports that predate the outcome years, though the three are not directly comparable with one another, for reasons the text explains.
\end{chapterabstract}
...
observed strictness at $\RubricSOfstedRaw$. Four further website-derived measures --- indicators of a traditional pedagogy and a traditional ethos, the school's religious character, and the prominence of faith in its self-presentation --- enter an exploratory section only. The previous chapter explains why the two dimensions need different instruments: warmth lives
...
circumstantial, and three pieces of it point the same way. A school's
...
consistent with stable culture without demonstrating it. The one cohort
that was measured at the same time as the fieldwork gives coefficients
within $0.03$ of the primary ones on the overall measure. And in the
national extension, old reports predict progress as well as recent ones:
...
bands), consistent with a signal that decays only slowly, though the marginal estimate is a decay and is read as such. Because the 2019 grade is held constant, the comparison is between
...
(\cref{tab:sensitivity_bounds} reports the no-control estimates). The
...
warmth, which no measure of classroom teaching could pick up.
...
$p = 0.56$), while the schools high on both dimensions sit $0.36$ of a
grade above the rest after controls. The additive description holds at
...
survive joint entry, and their counterparts do not. This suggests that the relational quality that goes with progress is the one visible in corridors and at break, and the order that goes with it the one visible in lessons, though the sub-scores are strongly collinear and the ranking of near-collinear siblings is fragile at this sample size.
...
associated with progress ($0.15$ per standard deviation), with the caveat
...
intervals are [$\NatWBootLo$, $\NatWBootHi$] for warmth and
[$NatTBootLo$, $NatTBootHi$] for teaching, both comfortably away from zero. Excluding the
visited schools from the national sample \emph{raises} the warmth
coefficient, so the association is no artefact of the schools that agreed
to take part.

The three numbers should be read against what each instrument is built to
carry, and not against one another. The benchmark for each leg is the
...
coefficient of $0.219$, roughly $0.07$, and $\NatW$ sits almost exactly
...
schools do steer pupils towards the academic pathway: nationally, the
strictness bands predict entry to the EBacc suite, to the humanities
and to languages, by between 1.6 and 3.5 percentage points per band standard
...
\input{tables/tab_identity} Schools whose websites espouse a traditional
...
remain significant on the total of the four buckets ($1.40$, $1.32$
points); on the English bucket alone warmth remains significant ($0.35$)
while strictness does not ($0.16$, $p = 0.12$). The Key Stage~2 control in
...
on the overall measure (\cref{tab:p8_proxy}); the contemporaneous strictness coefficient is if anything larger. That observation is consistent with the timing argument, but a same-year association is equally open to results shaping the school's orderliness, so it is not counted as evidence on its own. Restricting the sample to the 22 schools where the headteacher named in
the most recent inspection report is confirmed still in post in July 2026,
so that the culture observed is more plausibly the culture the outcome
cohorts had, leaves the strictness point estimates at or above the
full-sample values, while the warmth point estimate is close to zero, with
standard errors three to seven times the full-sample ones, and should not be
read directionally. That subsample is too small to be informative either
...
robustness. The informative statements for warmth are the bound and the
E-value: the coefficient that survives when unobservables are assumed to
matter exactly as much as the controls is \OsterBetaEqualW{}, and what the
convention cannot speak to is a confounder patterned unlike the observed
controls, working in the opposite direction to the EAL share. The
...
that does the work, and relational culture as secondary. These estimates give relational culture the same weight as discipline, they show no
...
outside the classroom, in corridors, at break and at the gate, which is, in the visited schools' accounts, where observation and training effort is least often directed. Both implications are conditional on the association being at
...
school predicts nothing on warmth. And the associations are not a
...
coefficient by 11 to 14~per~cent, so the entry channel carries little of the
association.
...
amplify; the national panel using the inspection-report scores detects a small
positive interaction ($0.016$), significant at that sample size but an
order of magnitude smaller than the strictness main effect and about half
the warmth one. \Cref{tab:gaps} examines
