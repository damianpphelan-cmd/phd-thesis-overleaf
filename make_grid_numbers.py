...
text_spine.csv, analysis_dataset.csv (controls), the LOO-prediction files and
the text corpus; writes snippets/grid_numbers.tex (macros) and
...
def main() -> None:
    ...
    for s, d in order:
        ...
        permp = pcell(s, tmap[d], col="perm_p")
        c = cells[(s, d)]
        lo, hi = c["ci"]
        rows.append(
            f"{src_name[s]} & {d.capitalize()} & {f3(model)} & "
            f"{fmt_p(permp)} & "
            f"{f3(c['r_dec'])} & [{f3(lo)}, {f3(hi)}] & "
...
schools ($n = """ + str(n_cell) + r"""$ in every cell: 101 scored documents,
