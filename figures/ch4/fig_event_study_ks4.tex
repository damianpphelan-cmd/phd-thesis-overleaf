...
\begin{singlespace}
\fignote{Notes: Each point is the coefficient on an indicator for a given cohort before or after the change in A-level economics provision, relative to the cohort just before the change, estimated on switcher schools only. The outcome is the mean
...
reported in Appendix Table~\ref{tab:eventstudy_counts}. A test that all the pre-change coefficients are jointly zero gives $p = 0.82$ for Panel A and $p = 0.68$ for Panel B, so there is no evidence that the intake was already changing before provision changed.}
\end{singlespace}
\end{figure}
