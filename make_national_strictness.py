...
def main() -> int:
    ...
    # Superseded, 1 Sep 2026. Settled by reading the two outputs:
    #   tab_national_strictness_full.tex is Table 3A.8, and the committed
    #     file carries the header "generated by analyse_band_extras.py".
    #     That script builds it from the marking-scheme bands; this one
    #     builds an older two-panel version from a7_estimates.csv (Stata,
    #     24 Aug), which is why the two differ.
    #   tab_national_strictness.tex is \input by no chapter.
    # So this script has no live output: one table has a current producer
    # and the other is not in the thesis. It is kept rather than deleted
    # because a7_estimates.csv is still the record of the Stata leg.
    if not args.check and not args.force:
        print("REFUSING TO WRITE. Superseded 1 Sep 2026: "
              f"{OUT_FULL.name} (Table 3A.8) is produced by "
              "analyse_band_extras.py from the marking-scheme bands -- the "
              "committed file carries that header -- and this script writes "
              f"an older two-panel version over it. {OUT.name} is not in the "
              "thesis. Run analyse_band_extras.py instead; --force to write.",
              file=sys.stderr)
        return 2
