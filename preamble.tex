...
% U+F06E is a private-use codepoint (a Wingdings bullet that survived a
% Word copy-paste) embedded in quoted Ofsted text in snippets/app_examples.tex.
% The snippet is generated, so the character would come back on regeneration;
% render it as nothing rather than letting pdfLaTeX stop with an error.
\DeclareUnicodeCharacter{F06E}{}
...
% A float page holds its content at the top rather than centring it
% vertically, so a table or figure that needs a page of its own does not
% sit in the middle of white space.
\makeatletter
\setlength{\@fptop}{0pt}
...
% --- Chapter 4 support (paper imported from the subject-choice Overleaf) ---
% The chapter's figures are drawn programmatically with pgfplots/TikZ rather
% than included as PDFs, and its body uses \FloatBarrier before the appendix.
\usepackage{pgfplots}
...
\usepackage{placeins}
% Macros the imported tables and figure notes use.
% \sym wraps significance stars; \redbf marks the paper's outstanding
% placeholders in red so they cannot be missed in the built PDF.
\newcommand{\sym}[1]{#1}
\newcommand{\redbf}[1]{\textbf{\color{red}#1}}
\newcommand{\fignote}[1]{\par\smallskip\footnotesize\noindent\textit{#1}}
\newcommand{\tabnote}[1]{\par\smallskip\footnotesize\noindent\textit{#1}}
% ---------------------------------------------------------------------------
...
% Unnumbered (front-matter) chapter headings sit closer to the top of the
% page and to the text below them than numbered chapter headings do. The
% report class puts 50pt above and 40pt below; 10pt and 20pt gain about two
% lines per page, which keeps the abstract and the acknowledgements from
% spilling a line or two onto an otherwise empty page.
\makeatletter
% Numbered chapter headings likewise start nearer the top of the page
% (10pt rather than 50pt above "Chapter N"), so that each paper's author
% footnote and abstract fit on its opening page.
